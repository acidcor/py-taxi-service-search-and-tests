...
class TestPublicPage(TestCase):
    ...
    def test_retrieve_cars_list(self):
        Car.objects.create(model="car2", manufacturer=Manufacturer.objects.create(name="Tesla"))
        Car.objects.create(model="car1", manufacturer=Manufacturer.objects.create(name="Tesla2"))
        response = self.client.get(CARS_LIST_URL)
        self.assertEqual(response.status_code, 200)
        car = Car.objects.all()
        self.assertEqual(
